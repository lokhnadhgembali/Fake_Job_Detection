...
import { UserPlus, Loader2, User, Mail, Lock, Shield } from "lucide-react";
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="bg-white p-8 md:p-10 text-center rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-slate-100 relative overflow-hidden">
          ...
          <div className="flex justify-center mb-6">
            <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center text-white shadow-lg transform -rotate-3 hover:rotate-0 transition-transform">
              <UserPlus className="w-8 h-8" />
            </div>
          </div>
          
          <h2 className="text-3xl font-bold tracking-tight text-slate-900 mb-2">Create Account</h2>
          <p className="text-slate-500 text-sm mb-8">Join JobCheck AI to track your scans</p>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            {success && (
              <div className="p-3 text-sm text-green-400 bg-green-400/10 border border-green-500/20 rounded-xl">
                {success}
              </div>
            )}
            
            <div className="text-left group relative">
              <label className="block text-sm font-medium text-slate-700 mb-1 ml-1" htmlFor="username">Username</label>
          ...
          <div className="mt-8 text-sm text-slate-500">
            ...
            <Link href="/login" className="font-medium text-blue-600 hover:text-purple-600 transition-colors">
              Log in instead
            </Link>
          </div>
        </div>
